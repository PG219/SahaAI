...
const I18N = {
  en: { appName:"FinAccess AI", role_user:"Customer", role_bank:"Bank Officer", tabs:{dashboard:"Dashboard",credit:"Credit Engine",risk:"Risk Forecast",products:"Products",rl:"RL Agent",finn:"FINNs"}, bankTabs:{overview:"Overview",applicants:"Applicants",portfolio:"Portfolio",fairness:"Fairness Audit",workflow:"Approvals"}, creditScore:"Credit Score", monthlyIncome:"Monthly Income", monthlyExpenses:"Monthly Expenses", financialRisk:"Financial Risk", repaymentHistory:"Repayment History (12 months)", liveSimulation:"Live Simulation — Adjust Parameters", aiAdvisor:"AI Financial Advisor", shapTitle:"XGBoost Credit Score — Feature Impact (SHAP)", hurts:"← Hurts score", helps:"Helps score →", fairnessTitle:"Algorithmic Fairness", dataInputs:"Data Inputs", riskTrend:"12-Month Risk Trend", riskDrivers:"Risk Drivers", nextMonth:"Next Month Prediction", distressProb:"Probability of financial distress", getActionPlan:"Get AI Action Plan ↗", products_title:"Matched Products", eligibility:"Eligibility match", explainElig:"Explain eligibility ↗", rlTitle:"RL Agent — Repayment Learning History", stateSpace:"State Space", rewardFn:"Reward Function", finnsTitle:"Finance-Informed Neural Network Constraints", finnVsML:"FINNs vs Classical ML", lossComponents:"Loss Function Components", explainFinn:"Explain FINNs concept ↗", live:"Live", aiQuestions:["What should I do to improve my credit score?","Am I at risk of financial distress?","Which loan product suits me best?"], totalApplicants:"Total Applicants", activeLoans:"Active Loans", avgScore:"Avg Credit Score", portfolioRisk:"Portfolio Risk", approveBtn:"Approve", flagBtn:"Flag", rejectBtn:"Reject", applicantList:"Applicant Queue", batchUpload:"Batch Score Upload", fairnessAudit:"Bias Detection Across Groups", approvalWorkflow:"Approval Workflow",
...
const T={bg:"#07090f",card:"#0f1623",cardAlt:"#111c2e",border:"#1a2840",gold:"#d4a843",teal:"#00c9a7",red:"#f25a5a",blue:"#4fa3e0",purple:"#a78bfa",text:"#dde6f0",muted:"#5b7085",dim:"#1e2d42"};
...
export default function App() {
  ...
  const [bankAuth, setBankAuth] = useState({institution:"State Bank Demo Branch",employeeId:"BK-2048",password:""});
  ...
  const MOCK_RESPONSES = {
    en: [
      ...
      { keys:["finn","neural","model","ai","algorithm","machine","xgboost","shap"],
        fn:()=>`Our AI uses a special model called FINNs — Finance-Informed Neural Networks. Unlike regular AI, it follows hard financial rules: for example, it will never suggest you borrow money if you have zero income. It also checks that your loan repayment never exceeds 40% of your salary. Think of it as an AI that went to finance school!` },
  ...
  const FALLBACK = {
    en: `I'm your FinAccess AI advisor! Your current profile: income ₹${(income/1000).toFixed(0)}k, expenses ₹${(expense/1000).toFixed(0)}k, credit score ${score}, risk ${riskPct}%. Try asking me about your credit score, loan options, savings tips, or financial risk — I'm here to help in simple language!`,
    hi: `मैं आपका FinAccess AI सलाहकार हूं! आपकी प्रोफाइल: आय ₹${(income/1000).toFixed(0)}k, खर्च ₹${(expense/1000).toFixed(0)}k, क्रेडिट स्कोर ${score}, जोखिम ${riskPct}%। मुझसे अपने क्रेडिट स्कोर, लोन, बचत, या जोखिम के बारे में पूछें!`,
    ta: `நான் உங்கள் FinAccess AI ஆலோசகர்! உங்கள் விவரம்: வருமானம் ₹${(income/1000).toFixed(0)}k, செலவு ₹${(expense/1000).toFixed(0)}k, மதிப்பெண் ${score}, இடர் ${riskPct}%. கடன் மதிப்பெண், கடன் தயாரிப்புகள், சேமிப்பு பற்றி கேளுங்கள்!`,
    te: `నేను మీ FinAccess AI సలహాదారుని! మీ వివరాలు: ఆదాయం ₹${(income/1000).toFixed(0)}k, ఖర్చులు ₹${(expense/1000).toFixed(0)}k, స్కోర్ ${score}, రిస్క్ ${riskPct}%. క్రెడిట్ స్కోర్, రుణాలు, పొదుపు గురించి అడగండి!`,
    bn: `আমি আপনার FinAccess AI উপদেষ্টা! আপনার তথ্য: আয় ₹${(income/1000).toFixed(0)}k, ব্যয় ₹${(expense/1000).toFixed(0)}k, স্কোর ${score}, ঝুঁকি ${riskPct}%। ক্রেডিট স্কোর, ঋণ, সঞ্চয় নিয়ে জিজ্ঞেস করুন!`,
    kn: `ನಾನು ನಿಮ್ಮ FinAccess AI ಸಲಹೆಗಾರ! ನಿಮ್ಮ ವಿವರ: ಆದಾಯ ₹${(income/1000).toFixed(0)}k, ವೆಚ್ಚ ₹${(expense/1000).toFixed(0)}k, ಸ್ಕೋರ್ ${score}, ರಿಸ್ಕ್ ${riskPct}%. ಕ್ರೆಡಿಟ್ ಸ್ಕೋರ್, ಸಾಲ, ಉಳಿತಾಯ ಬಗ್ಗೆ ಕೇಳಿ!`,
    mr: `मी तुमचा FinAccess AI सल्लागार आहे! तुमची माहिती: उत्पन्न ₹${(income/1000).toFixed(0)}k, खर्च ₹${(expense/1000).toFixed(0)}k, स्कोअर ${score}, जोखीम ${riskPct}%. क्रेडिट स्कोअर, कर्ज, बचत याबद्दल विचारा!`,
  };
  ...
  const Dashboard = () => (
    <div style={sec}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))",gap:12,marginBottom:12}}>
        ...
        {[[t.monthlyIncome,`₹${(income/1000).toFixed(0)}k`,T.teal,RISK_HISTORY.map(d=>d.income/1000)],[t.monthlyExpenses,`₹${(expense/1000).toFixed(0)}k`,T.gold,RISK_HISTORY.map(d=>d.expense/1000)],[t.financialRisk,`${riskPct}%`,riskPct>40?T.red:T.teal,RISK_HISTORY.map(d=>d.risk)]].map(([lbl,val,col,data])=>(
      ...
      <div style={{...card,marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))",gap:20,marginTop:10}}>
          {[[t.monthlyIncome,income,setIncome,10000,80000,T.teal],[t.monthlyExpenses,expense,setExpense,5000,70000,T.gold]].map(([lbl,val,set,min,max,col])=>(
            <div key={lbl}><div style={{display:"flex",justifyContent:"space-between",fontSize:14,color:T.muted,marginBottom:8}}><span>{lbl}</span><span style={{color:col,fontFamily:"monospace",fontSize:15}}>₹{(val/1000).toFixed(0)}k</span></div><input type="range" min={min} max={max} step={1000} value={val} onChange={e=>set(+e.target.value)} style={{width:"100%",accentColor:col}}/></div>
          ))}
        </div>
      </div>
      <div style={{...card,marginBottom:12}}>
  ...
  const Credit = () => (
    <div style={sec}><div style={g2}>
      <div style={card}><div style={label}>{t.shapTitle}</div><div style={{marginTop:14}}><div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:T.muted,marginBottom:10}}><span>{t.hurts}</span><span>{t.helps}</span></div>{SHAP_FEATURES.map(f=><ShapBar key={f.name} {...f}/>)}</div><div style={{marginTop:12,paddingTop:10,borderTop:`1px solid ${T.border}`,fontSize:11,color:T.muted}}>Model: XGBoost · Explainability: SHAP · Fairness: Fairlearn</div></div>
  ...
  const FINNs = () => (
    <div style={sec}><div style={g2}>
      <div style={card}><div style={label}>{t.finnsTitle}</div><div style={{fontSize:11,color:T.muted,marginBottom:12}}>Hard economic rules baked into the model loss function.</div>{FINN_C.map((c,i)=><div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,padding:"9px 12px",background:c.status==="warn"?T.gold+"0d":T.dim,border:`1px solid ${c.status==="warn"?T.gold+"44":T.border}`,borderRadius:7}}><div style={{flex:1}}><div style={{fontSize:12}}>{c.rule}</div><div style={{fontSize:10,color:T.muted,marginTop:1}}>Actual: <span style={{fontFamily:"monospace",color:c.status==="warn"?T.gold:T.text}}>{c.actual}</span> · {c.threshold}</div></div><span style={{...badge,marginLeft:10,background:c.status==="warn"?T.gold+"22":T.teal+"22",color:c.status==="warn"?T.gold:T.teal}}>{c.status}</span></div>)}</div>
      <div><div style={{...card,marginBottom:12}}><div style={label}>{t.finnVsML}</div>{[{label:"Respects financial axioms",finn:true,ml:false},{label:"Zero-income → zero credit",finn:true,ml:false},{label:"Debt cap enforcement",finn:true,ml:false},{label:"Learns from data",finn:true,ml:true},{label:"Handles rare edge cases",finn:true,ml:false}].map(r=><div key={r.label} style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:7,padding:"5px 10px",background:T.dim,borderRadius:5}}><span style={{fontSize:12,flex:1}}>{r.label}</span><span style={{fontSize:11,color:r.finn?T.teal:T.red,fontFamily:"monospace",minWidth:50,textAlign:"center"}}>{r.finn?"✓ FINN":"✗ FINN"}</span><span style={{fontSize:11,color:r.ml?T.teal:T.red,fontFamily:"monospace",minWidth:38,textAlign:"center"}}>{r.ml?"✓ ML":"✗ ML"}</span></div>)}</div>
      <div style={card}><div style={label}>{t.lossComponents}</div><div style={{padding:"10px 12px",background:T.dim,borderRadius:7,fontFamily:"monospace",fontSize:11,lineHeight:2}}><div><span style={{color:T.gold}}>L_total</span> = L_data</div><div style={{paddingLeft:12}}>+ <span style={{color:T.red}}>λ₁</span>·L_exp_cap + <span style={{color:T.red}}>λ₂</span>·L_dti_ratio</div><div style={{paddingLeft:12}}>+ <span style={{color:T.red}}>λ₃</span>·L_zero_income + <span style={{color:T.red}}>λ₄</span>·L_repay_floor</div></div><button onClick={()=>askAI("What are Finance-Informed Neural Networks and why are economic constraints important in financial ML?")} style={{marginTop:10,width:"100%",background:T.gold+"11",border:`1px solid ${T.gold}33`,borderRadius:6,padding:"7px",color:T.gold,fontSize:11,cursor:"pointer"}}>{t.explainFinn}</button></div></div>
    </div>
    <AIAdvisorBlock accentColor={T.gold}/>
    </div>
  );
  ...
  const BankPortfolio = () => (
    <div style={sec}><div style={g2}>
      ...
      <div style={card}><div style={label}>Federated Learning Status</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:10}}>{[{bank:"SBI Branch",status:"synced",rounds:142,color:T.teal},{bank:"PNB Branch",status:"synced",rounds:138,color:T.teal},{bank:"BoB Branch",status:"training",rounds:135,color:T.gold},{bank:"UCO Bank",status:"pending",rounds:112,color:T.muted}].map(b=><div key={b.bank} style={{background:T.dim,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:11,fontWeight:600,marginBottom:4}}>{b.bank}</div><span style={{...badge,background:b.color+"22",color:b.color}}>{b.status}</span><div style={{fontSize:10,color:T.muted,marginTop:4}}>Rounds:{b.rounds}</div></div>)}</div><div style={{marginTop:10,fontSize:11,color:T.muted}}>Global accuracy: <span style={{color:T.teal,fontFamily:"monospace"}}>87.3%</span></div></div>
    </div></div>
  );
      ...
      <div style={{...sec,maxWidth:1240,paddingTop:48,paddingBottom:48}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,gap:16,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:280}}>
            <div style={{fontSize:13,letterSpacing:2,color:T.gold,textTransform:"uppercase",marginBottom:10}}>FinAccess AI</div>
            <div style={{fontSize:36,fontWeight:700,lineHeight:1.15,maxWidth:640,marginBottom:12}}>Financial guidance for everyone — including those who cannot read</div>
            <div style={{fontSize:15,color:T.muted,lineHeight:1.7,maxWidth:680}}>Speak your question in Hindi, Tamil, Telugu, Bengali, Kannada, Marathi or English. Our AI listens and answers you back — in your own voice.</div>
          </div>
        ...
        <div style={{...cardAlt,marginBottom:20,borderLeft:`3px solid ${T.teal}`,display:"flex",gap:20,alignItems:"center",flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:200}}>
            <div style={{fontSize:14,fontWeight:700,color:T.teal,marginBottom:6}}>🎤 No reading needed</div>
            <div style={{fontSize:13,color:T.muted,lineHeight:1.65}}>Just tap the microphone and speak. Our AI will understand your language and read the answer back to you aloud.</div>
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(320px, 1fr))",gap:20}}>
          {[
            {accent:T.gold,emoji:"👤",role:"Customer Portal",title:"Borrow · Save · Grow",desc:"Know your credit score, find the right loan, and plan your finances — in your own language.",features:["🎤 Speak your question, hear the answer","📊 Personal credit dashboard","🔊 All answers read aloud"],label:"Continue as User →",fn:()=>beginAuth("user")},
            {accent:T.blue,emoji:"🏦",role:"Bank Portal",title:"Review · Score · Approve",desc:"Applicant queue, batch scoring, fairness audits, and approval workflows for bank officers.",features:["🔐 Secure employee credentials","📋 Applicant review queue","⚖️ Fairness & bias audit"],label:"Continue as Bank Officer →",fn:()=>beginAuth("bank")}
        ...
        <div style={{...sec,maxWidth:1120,paddingTop:48,paddingBottom:48}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(340px, 1fr))",gap:24}}>
            <div style={{...cardAlt,border:`1px solid ${accent}44`,padding:28,display:"flex",flexDirection:"column",gap:16}}>
              ...
              <div style={{fontSize:28,fontWeight:700,lineHeight:1.2}}>{isUser?"Your money, your voice":"Secure officer workspace"}</div>
              <div style={{fontSize:14,color:T.muted,lineHeight:1.7}}>{isUser?"No need to type — tap the mic and say your name. We understand all Indian languages.":"Authenticate with your institution credentials."}</div>
            ...
            <div style={{...card,display:"grid",gap:16,alignContent:"start"}}>
              ...
              {isUser?<>
                ...
                <div><div style={{fontSize:13,color:T.muted,marginBottom:8}}>OTP</div><input value={authState.otp} onChange={e=>setAuthState(v=>({...v,otp:e.target.value}))} style={authInput} placeholder="Enter demo OTP"/></div>
              </>:<>
              ...
              <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
                <button onClick={()=>completeAuth(mode)} style={{background:accent,border:"none",borderRadius:10,padding:"12px 20px",color:"#07090f",fontSize:15,fontWeight:700,cursor:"pointer"}}>{isUser?"Enter Portal":"Enter Bank Portal"}</button>
